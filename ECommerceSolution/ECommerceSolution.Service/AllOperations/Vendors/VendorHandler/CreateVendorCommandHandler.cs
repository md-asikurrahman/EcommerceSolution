...
{
    public class CreateVendorCommandHandler : IRequestHandler<CreateVendorCommand, List<string>>
    {
        public async Task<List<string>> Handle(CreateVendorCommand request, CancellationToken cancellationToken)
        {
            NotificationModel nModel = new NotificationModel();
            DataSet dataSet = new DataSet();
            ParameterList parameterList = new ParameterList();
            string storedProcedureName = "SP_Create_Vendor";
            parameterList.Add("@VendorName", request.VendorName!, SqlDbType.NVarChar);
            parameterList.Add("@VendorTypeId",Convert.ToString( request.VendorTypeId), SqlDbType.NVarChar);
